...
fn check_round_trip(array: Box<dyn Array>) -> Result<Vec<Box<dyn Array>>,Error> {
    ...
    // Safety: `write` above guarantees that this is a valid Arrow IPC file
    let new_chunk = unsafe { arrow2::mmap::mmap_unchecked(&metadata, &dictionaries, data, 0)? };
    // assert_eq!(new_array, array);
    Ok(new_chunk.into_arrays())
}

fn main() -> Result<(), Error> {
    let array = Int32Array::from([None, None, Some(123)]).boxed();
    let mut dyn_array = check_round_trip(array)?;
    let mut arr = dyn_array[0].as_any_mut().downcast_mut::<Int32Array>();
    if let Some(arr) = arr {
        let vals = arr.get_mut_values();
        println!("Can we really write to this? {:?}", vals);
    }
    Ok(())
}
